...
from chatgpt import yandex_gpt_manager, gpt_manager as gpt
    ...
    if ResultsList[0] != "Нет результатов":
        try:
           # st.write(f'Краткий ответ: {gpt.fetch_gpt_response(query, ResultsList[0]+ResultsList[1]+ResultsList[2])}') #OpenAI
            st.write(f'Краткий ответ: {yandex_gpt_manager.fetch_yandex_gpt_response(query, ResultsList[0] + ResultsList[1] + ResultsList[2])}') #Yandex GPT
        except:
           # st.write('Краткий ответ: ', gpt.fetch_gpt_response(query, ResultsList[0])) #OpenAI
            st.write('Краткий ответ: ', yandex_gpt_manager.fetch_yandex_gpt_response(query, ResultsList[0])) #Yandex GPT
        st.write("------------------")
